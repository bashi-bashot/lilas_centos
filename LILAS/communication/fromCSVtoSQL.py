...
def createAppel(t, listeLif):
    ...
    for k in range(len(t)):
        ...
        print("Taille du filtre : "+date_a_sauvegarder.Appel.all().filter(heure__exact=d.time()).filter(appelant__exact=apple).filter(line_appelante__exact=fsx_e).filter(appele__exact=applant).filter(line_appele__exact=fsx_a).count())
