...
for i in range(len(tabIndex)):
    ...
    if NumExterieur.objects.filter(numero__contains=str(t[tabIndex[i]][32])).filter(nom__contains=t[tabIndex[i]][5]).count()==0:
        corr = NumExterieur(id_elts =id , id_lilas = str(i), numero = str(t[tabIndex[i]][32]), nom = t[tabIndex[i]][5])
        corr.save()
    else :
...
for i in range(len(tabIndexCNUM)):
    ...
    if NumExterieur.objects.filter(numero__contains=str(t[tabIndexCNUM[i]][26])).filter(nom__contains=t[tabIndexCNUM[i]][5]).count()==0:
        corr = NumExterieur(id_elts =id , id_lilas = str(i), numero = str(t[tabIndexCNUM[i]][26]), nom = t[tabIndexCNUM[i]][5])
        corr.save()
    else :
        print("Correspondant deja existant")
...
for i in range(len(tabIndexGRP)):
    ...
    if NumExterieur.objects.filter(numero__contains=str(t[tabIndexGRP[i]][32])).filter(nom__contains=t[tabIndexGRP[i]][5]).count()==0:
        corr = NumExterieur(id_elts =id , id_lilas = str(i), numero = str(t[tabIndexGRP[i]][32]), nom = t[tabIndexGRP[i]][5])
        corr.save()
    else :
        print("Correspondant deja existant")
